...
data_x = np.load(os.path.join(SPEECH_DATA_PATH, '{}.npy'.format(mode)), encoding='bytes')
data_y = np.load(os.path.join(SPEECH_DATA_PATH, '{}_transcripts.npy'.format(mode)), encoding='bytes')
...
np.save(os.path.join(DUMP_DATA_PATH, '{}_transcripts.npy'.format(mode)), data_y_rev)
