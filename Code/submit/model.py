...
class Encoder(nn.Module):
    ...
    def __make_layer__(self, in_dim, out_dim):
        ...
        # return lstm
        return WeightDrop(lstm, ['weight_hh_l0','weight_hh_l0_reverse'], dropout=0.5)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, inputs, words, TF):
        ...
        prediction = torch.zeros(max_len, batch_size, self.vocab_dim).to(self.device)
        ...
        for t in range(max_len):
            ...
            word_vec, hidden1, cell1, hidden2, cell2 = self.decoder(word, context, hidden1,
                                                                    cell1, hidden2, cell2, first_step=(t==0))
            prediction[t] = word_vec
            teacher_force = torch.rand(1) < TF
            if teacher_force:
                word = words[t]
            else:
                gumbel = torch.autograd.Variable(self.sample_gumbel(shape=word_vec.size(),
