...
data = np.load(os.path.join(SPEECH_DATA_PATH, '{}_transcripts.npy'.format(mode)), encoding='bytes')
...
print('Number of duplicate instances in %s data: %d/%d, %.2f %%' % (mode, len(duplicates), len(data), per))
